...
bool GetLandmark(const tf::TransformListener& tf_listener,
                 const std::string& id, PointCloud<PointXYZRGB>::Ptr landmark);
void GetScene(const tf::TransformListener& tf_listener,
              PointCloud<PointXYZRGB>::Ptr scene);
...
PointCloud<PointXYZRGB>::Ptr CropScene(PointCloud<PointXYZRGB>::Ptr scene);
...
int main(int argc, char** argv) {
  ...
  ROS_INFO("Getting landmark");
  PointCloud<PointXYZRGB>::Ptr landmark_cloud(new PointCloud<PointXYZRGB>());
  bool success =
      GetLandmark(tf_listener, "577dca1e5163504a3d47a495", landmark_cloud);
  if (!success) {
    return 1;
  }
  PointCloud2 landmark_msg;
  pcl::toROSMsg(*landmark_cloud, landmark_msg);
  ...
  GetScene(tf_listener, scene_cloud);

  std::string user_input;
  bool debug = false;
  while (true) {
    std::cout << "Type \"debug\" or just press enter ";
    ...
    PointCloud<PointXYZRGB>::Ptr cloud = CropScene(scene_cloud);
    ...
    bool found = FindLandmark(cloud, landmark_cloud, found_landmark, debug);
    if (!found) {
...
bool GetLandmark(const tf::TransformListener& tf_listener,
                 const std::string& id,
                 PointCloud<PointXYZRGB>::Ptr landmark_cloud) {
  ...
  Landmark landmark = res.landmark;
...
bool FindLandmark(PointCloud<PointXYZRGB>::Ptr scene,
  ...
  //  *inlier_frac =
  //      align.getInliers().size() / static_cast<double>(landmark->size());
  //  return true;
  //} else {
  //  return false;
  //}
...
void GetScene(const tf::TransformListener& tf_listener,
  ...
  pcl::fromROSMsg(transformed_scene, *scene);
  ROS_INFO("Loaded scene with %ld points", scene->size());
}
...
PointCloud<PointXYZRGB>::Ptr CropScene(PointCloud<PointXYZRGB>::Ptr scene) {
  ...
  PointCloud<PointXYZRGB>::Ptr cropped_scene(new PointCloud<PointXYZRGB>());
  ...
  crop.setMin(min);
  crop.setMax(max);
  crop.filter(*cropped_scene);
  ROS_INFO("Cropped to %ld points", cropped_scene->size());
